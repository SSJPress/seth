...
public class FinalTest {
    ...
    private void testRedSeven( int expectedPlayerValue, int expectedDealerValue, double expectedBankroll, Shoe shoe, boolean surrender, boolean buyInsurance, int... expectedPlayerValues) {
        ...
        Main.setSURRENDER(surrender);
        Main.setSPLITNUM(2);
        Main.setBuyInsurance(buyInsurance);
        
        Main.setWallet(START);
        Main.setBetnum(BET);
        Main.setDeck(shoe);
        Main.autoPlay();
        Main.dealerTurn(Main.getpScores(), Main.getpChoices());
        Main.results(Main.getpScores(), Main.getpChoices(), Main.getDealerHand().getValueInt());
